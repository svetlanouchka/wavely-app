...
export default function FrequencyPage() {
			...
			if (tags.length > 0 && frequency) {
				...
				const getFrequenciesForTag = async (randomTag) => {
					const allFrequencies = await fetch(
						...
						.catch((error) =>
							console.error(
								"Erreur de chargement des fréquences par tag",
								error,
							),
						);

					const filteredFrequencies = allFrequencies.filter(
						(freq) => freq.id !== frequency.id,
					);

					while (
						randomFrequencies.length < 3 &&
						filteredFrequencies.length > 0
					) {
						const randomIndex = Math.floor(
							Math.random() * filteredFrequencies.length,
						);
						randomFrequencies.push(
							filteredFrequencies.splice(randomIndex, 1)[0],
						);
					}
				};
	...
	return (
		<div className="flex flex-col min-h-screen">
			...
			<div className="flex flex-col md:flex-row items-center md:gap-20 gap-10 p-10">
				...
				<div className="flex w-full h-full">
					<div className="flex flex-col items-start text-left max-w-lg">
						<h1 className="text-3xl font-albert-sans text-left t-8">
							{frequency?.name}
						</h1>
						<p className="text-gray-600 font-albert-sans mt-4">
			...
			<div className="p-10">
				<h2 className="text-2xl font-albert-sans text-left">
					Essayez ces ondes
				</h2>
				<div className="flex flex-wrap gap-10 pt-10">
					{recommendedFrequencies.map((freq) => (
						<div
							key={freq.id}
							className="bg-gray-light w-70 h-70 shadow-lg rounded-sm p-2 flex flex-col items-center text-start"
						>
							<h2 className="text-lg font-albert mt-4">{freq.name}</h2>
							<img
								src={freq.image_url}
								alt={freq.name}
								className="w-50 h-50 hover:scale-110 duration-300 ease-in object-cover"
							/>
						</div>
					))}
				</div>
