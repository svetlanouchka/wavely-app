...
class SessionManager extends AbstractManager {
	...
	findAllSessionsByUserId(id) {
		return this.database.query(
			`SELECT s.id, s.created_at, s.note_before, s.note_after, s.comment, f.name, f.description, f.image_url FROM ${this.table} AS s JOIN frequency AS f ON f.id = s.frequency_id WHERE s.user_id = ?;`,
			[id],
		);
	}
}
